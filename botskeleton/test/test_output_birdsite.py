...
def test_birdsite_activates_correctly(testdir: str, credentials: str) -> None:
    bs = botskeleton.BotSkeleton(secrets_dir=testdir)

    assert(bs.outputs["birdsite"]["active"])

def test_birdsite_activates_with_owner_id_correctly(testdir: str, credentials: str) -> None:
    # no owner set
    bs = botskeleton.BotSkeleton(secrets_dir=testdir)
